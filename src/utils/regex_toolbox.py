...
from .list_toolbox import (
    create_list_from_elements_count,
    get_elements_by_indices,
    remove_empty,
    remove_nones,
)
from .tuples_list_toolbox import filter_column, get_column_as_list
from .txt_toolbox import load_text


def get_first_element_from_list_matching_regexs(
    ls, regexs, ignore_case=False, whole_word=False
):
    try:
        return next(
            s
...
def remove_elements_from_list_matching_regexs(
    ls, regexs, ignore_case=False, whole_word=False
):
    def f(s):
        return not matches_any_regex(
            s, regexs, ignore_case=ignore_case, whole_word=whole_word
        )
...
def remove_elements_from_tuples_list_matching_regexs(
    ls, i, regexs, ignore_case=False, whole_word=False
):
    def f(s):
        return not matches_any_regex(
            s, regexs, ignore_case=ignore_case, whole_word=whole_word
...
def remove_elements_from_list_not_matching_regexs(
    ls, regexs, ignore_case=False, whole_word=False
):
    def f(s):
        return matches_any_regex(
            s, regexs, ignore_case=ignore_case, whole_word=whole_word
        )
...
def remove_elements_from_tuples_list_not_matching_regexs(
    ls, i, regexs, ignore_case=False, whole_word=False
):
    def f(s):
        return matches_any_regex(
...
def remove_terms_not_in_text(text, terms, ignore_case=False):
    return list(
        filter(
            lambda term: contains_regex(
                text, make_literal(term.replace(" ", "\\s")), ignore_case=ignore_case
            ),
            terms,
        )
    )
...
def get_lines_indices_containing_literal(
    s, literal, ignore_case=False, whole_word=False
):
...
def get_lines_containing_literal(
    s, literal, ignore_case=False, whole_word=False, number_lines=False
):
    literal = make_literal(literal)
    return get_lines_containing_regex(
        s,
        literal,
...
def get_lines_containing_regex(
    s, regexs, ignore_case=False, whole_word=False, number_lines=False
):
    if isinstance(regexs, str):
        regexs = [regexs]
    matches = []
    ...
    for i, line in enumerate(lines):
        if matches_any_regex(
            line, regexs, ignore_case=ignore_case, whole_word=whole_word
        ):
            if number_lines:
...
def get_all_lines_matching_regexs(s, regexs, ignore_case=False):
    ...
    lines = s.splitlines()
    # matches = list(
    #     filter(lambda l: matches_any_regex(l, regexs, ignore_case=ignore_case), lines)
    # )
    for line in lines:
...
def remove_all_lines_matching_regexs(s, regexs, ignore_case=False):
    if isinstance(regexs, str):
        regexs = [regexs]
    lines = s.splitlines()
    not_matching_lines = list(
        filter(
            lambda l: not matches_any_regex(l, regexs, ignore_case=ignore_case), lines
        )
    )
...
def add_to_regexs(s, regexA, regexB, ignore_case=False):
    ...
    if len(blocks) > 1:
        ...
        match, i0, i1 = find_match_full(s, regexA, ignore_case=ignore_case)
...
def extract_fragment_full(
    s,
    regex0,
    regex1,
    include_opening_match=True,
    include_closing_match=False,
    ignore_case=False,
    whole_word=False,
):
    _, i0, match0 = get_from_match_full(
        s,
        regex0,
        ignore_case=ignore_case,
        whole_word=whole_word,
        include_match=include_opening_match,
    )
    i = i0 + len(match0)
    _, i1_, _ = get_until_match_full(
        s[i:],
        regex1,
        ignore_case=ignore_case,
        whole_word=whole_word,
        include_match=include_closing_match,
    )
    if i0 != -1 and i1_ != -1:
        i1 = i + i1_
    else:
...
def get_from_match_full(
    s, regex, ignore_case=False, whole_word=False, include_match=True
):
...
def get_until_match_full(
    s, regex, ignore_case=False, whole_word=False, include_match=False
):
    _, i0, i1 = find_match_full(
        s, regex, ignore_case=ignore_case, whole_word=whole_word
    )
...
def extract_fragment(
    ...
    s, _, _ = extract_fragment_full(
        s,
        regex0,
        regex1,
        include_opening_match=include_opening_match,
        include_closing_match=include_closing_match,
        ignore_case=ignore_case,
        whole_word=whole_word,
    )
    return s
...
def contains_unix_regex(s, regex, ignore_case=False, whole_word=False):
    regex = regex2unixregex(regex)
    return contains_regex(s, regex, ignore_case=ignore_case, whole_word=whole_word)


def regex2unixregex(regex):
    return make_literal(regex).replace("*", ".*")


def find_match(s, regexs, ignore_case=False):
    if not isinstance(s, str):
        raise TypeError(f"Invalid type {type(s)} for input argument. Must be a string.")
    if isinstance(regexs, str):
        regexs = [regexs]
    if ignore_case:
        flags = re.MULTILINE | re.IGNORECASE
...
def find_match_full(
    s, regexs, ignore_case=False, whole_word=False, i_start=0, i_end=-1
):
    if not isinstance(s, str):
        raise TypeError(f"Invalid type {type(s)} for input argument. Must be a string.")
    if isinstance(regexs, str):
...
def find_full_regex_match(
    s, regex, ignore_case=False, whole_word=False, i_start=0, i_end=-1
):
    if ignore_case:
        flags = re.MULTILINE | re.IGNORECASE
    else:
...
def get_first_roman_number_full(s):
    ...
    if number is not None:
        while not (
            number.startswith("I")
            or number.startswith("V")
            or number.startswith("X")
            or number.startswith("L")
            or number.startswith("C")
            or number.startswith("D")
            or number.startswith("M")
        ):
            number = number[1:]
            i0 += 1
        while not (
            number.endswith("I")
            or number.endswith("V")
            or number.endswith("X")
            or number.endswith("L")
            or number.endswith("C")
            or number.endswith("D")
            or number.endswith("M")
        ):
            number = number[:-1]
            i1 -= 1
    return number, i0, i1
...
def is_numeral(s, language="en"):
    if isinstance(s, int) or isinstance(s, float):
        ...
        raise TypeError(
            f"Invalid type <{type(s)}> for input argument <s>. Valid types are: <str>,\
				<int> and <float>."
        )
...
def get_first_date(s):
    regexs = [
        "\\d+ de (enero|febrero|marzo|abril|mayo|junio|julio|agosto|septiembre|\
			octubre|noviembre|diciembre)( (de|del|del año) \\d{4})?",
        "\\d+(\\.|/|-)\\d+(\\.|/|-)\\d{4}",
    ]
...
def find_last_match_full_forward_strategy(
    s, regexs, ignore_case=True, whole_word=False
):
    match_full = (None, -1, -1)
    matches_full = find_all_matches_full(
        s, regexs, ignore_case=ignore_case, whole_word=whole_word
...
def find_last_match_full_backwards_strategy(
    s, regexs, step_size=1, ignore_case=True, whole_word=False
):
...
def find_last_match_full(
    s, regexs, ignore_case=True, whole_word=False, step_size=20, strategy="backwards"
):
    if strategy == "forward":
...
def remove_not_listed_characters(s, char_regexs, ignore_case=False):
    return "".join(
        filter(
            lambda c: matches_any_regex(
                c, char_regexs, ignore_case=ignore_case, whole_word=False
            ),
            s,
        )
    )
