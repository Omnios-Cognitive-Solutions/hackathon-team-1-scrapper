...
def get_webdriver(
    url: str, page_timeout: int = 60, browser=Browser.CHROME, headless: bool = False
) -> tuple[WebDriver, Display]:
    match browser:
        case Browser.CHROME:
            return get_chrome_webdriver(
                url, page_timeout=page_timeout, headless=headless
            )
        case Browser.FIREFOX:
            return get_firefox_webdriver(
                url, page_timeout=page_timeout, headless=headless
            )
        case _:
            raise Exception(f"Invalid browser. Valid values are: {list(Browser)}")
    # try:
...
def get_chrome_webdriver(
    url: str, page_timeout: int = 60, headless: bool = False
) -> tuple[WebDriver, Display]:
...
def wait_for_element(
    ...
    try:
        ...
        for i in range(0, max_attempts):
            try:
                element_present = EC.presence_of_element_located((element_type, value))
...
def click_dropdown_option_by_text(
    dropdown_element, value, ignore_case: bool = False
) -> None:
    for option in dropdown_element.find_elements_by_tag_name("option"):
        if option.text == value or (
            ignore_case and option.text.lower() == value.lower()
        ):
...
class BaseSeleniumCrawler:
    ...
    def find_element(
        self, by: str, value: str, max_attempts=None, retry_wait_time=None
    ) -> WebElement:
